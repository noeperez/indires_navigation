...
Local3DPlanner::Local3DPlanner(
    std::string name, tf2_ros::Buffer* tf, local_3d_planner::OdometryHelperRos* oh,
    // std::vector<geometry_msgs::Point> footprint_spec,
    double robot_radius, double local_area_radius, double controller_freq,
    double max_trans_vel, double min_trans_vel, double max_rot_vel, double min_rot_vel,
    ...
    double sim_time, double sim_granularity, double angular_sim_granularity, bool dwa)
{
  ...
  collision_detector_ = new CollisionDetection(
      name, tf, oh, max_vel_x_, max_vel_th_, acc_lim_trans_, acc_lim_rot_, sim_time_,
      robot_radius_, local_area_radius_, sim_granularity_);
}
...
bool Local3DPlanner::findBestAction(geometry_msgs::PoseStamped global_pose,
                                    geometry_msgs::PoseStamped global_vel, geometry_msgs::Twist& cmd_vel)
{
  ...
  rz = global_pose.pose.position.z;
  rt = tf::getYaw(global_pose.pose.orientation);
  rvx = global_vel.pose.position.x;
  rvy = global_vel.pose.position.y;
  rvt = tf::getYaw(global_vel.pose.orientation);


  // Check if we are close enough to the goal
  double dist_goal = sqrt((rx - goal_x_) * (rx - goal_x_) +
                          (ry - goal_y_) * (ry - goal_y_));  //+(rz-goal_z_)*(rz-goal_z_));
  double dist_start = sqrt((rx - start_x_) * (rx - start_x_) +
                           (ry - start_y_) * (ry - start_y_));  //+(rz-start_z_)*(rz-start_z_));
  ...
  // double dz = (wpz - rz);  //???
  // features_->transformPoseTo(geometry_msgs::PoseStamped pose_in, std::string frame_out,
  // bool usetime);
  ...
  else  // Select the linear and angular velocities to reach the way-point
  {
    ...
    // Set the sign of the commanded angular velocity and reset in case of small
    // variations
  ...
  else if (dwa_)
  {
    ...
    for (unsigned int l = 0; l <= 3; l++)
    {
      ...
      for (unsigned int v = 1; v <= 4; v++)
      {
        ...
        bool valid1 =
            collision_detector_->checkTraj(rvx, rvy, rvt, vx, vy, vt, px, py, pz, pth);
        double d1 = 0.0;
        if (valid1)
        {
          d1 =
              sqrt((dx - px) * (dx - px) + (dy - py) * (dy - py));  // + (dz-pz)*(dz-pz));
          best_vels.vel_th = vt;
        }
        ...
        if (valid2)
        {
          ...
          if (valid1)
          {
            double d2 = sqrt((dx - px) * (dx - px) +
                             (dy - py) * (dy - py));  // + (dz-pz)*(dz-pz));
            if (d2 < d1)
            {
              best_vels.vel_th = vt;
            }
        ...
        if (valid1 || valid2)
        {
          ...
          // double secs = (t2-t1).toSec();
          // printf("\n\nFindBestPathTime: %f seconds\n\n", secs);
          // printf("FindBestAction. Valid Traj found with DWA. vx:%.2f, vy:%.2f,
...
// we need to take the footprint of the robot into account when we calculate cost to
// obstacles
/*double UpoPlanner::footprintCost(double x_i, double y_i, double theta_i){
  //check if the footprint is legal
  return world_model_.footprintCost(x_i, y_i, theta_i, footprint_spec_, inscribed_radius_,
circumscribed_radius_);
}*/
};

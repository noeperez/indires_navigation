...
upo_RRT_ros::RRT_ros_wrapper3::RRT_ros_wrapper3() : tf_(NULL) {}



upo_RRT_ros::RRT_ros_wrapper3::RRT_ros_wrapper3(tf::TransformListener* tf)
{
	tf_ = tf;
	rrt_planner_ = NULL;
...
void upo_RRT_ros::RRT_ros_wrapper3::setup()
{
	...
	//CNN path prediction biasing
	private_nh.param<bool>("prediction_biasing", prediction_biasing_, false);
	private_nh.param<double>("prediction_bias", aux, 0.8);
	prediction_bias_ = (float)aux;
	private_nh.param<double>("prediction_stddev", aux, 0.3);
	prediction_stddev_ = (float)aux;
	...
	checker_ = new ValidityChecker3(tf_, xy_res_, size_x_, size_y_, dimensions_, distanceType_);
	...
	if(prediction_biasing_) {
		rrt_planner_->setFullBiasing(prediction_biasing_);
		rrt_planner_->setPathBias(prediction_bias_);
		rrt_planner_->setPathBias_stddev(prediction_stddev_);
	}

	
	//Planning server
	ros::NodeHandle nhandle("RRT_ros_wrapper3");
	plan_srv_ = nhandle.advertiseService("makeRRTPlan", &upo_RRT_ros::RRT_ros_wrapper3::makePlanService, this);
...
std::vector<geometry_msgs::PoseStamped> upo_RRT_ros::RRT_ros_wrapper3::RRT_plan(geometry_msgs::Pose2D start, geometry_msgs::Pose2D goal, float start_lin_vel, float start_ang_vel)
{
	...
	//sleep(2);
	if(visualize_costmap_)
		checker_->publish_costmap_ros(time);

	if(prediction_biasing_)
		setPathPredictionBiasing();
	...
	if(path.empty()) {
		printf("Wrapper3. Path planned is empty!!!\n");
		return rrt_plan_; 
	}
	if(show_statistics_) {
...
bool upo_RRT_ros::RRT_ros_wrapper3::makePlanService(upo_rrt_planners::MakePlan::Request &req, upo_rrt_planners::MakePlan::Response &res)
{
	//printf("Wrapper. MakePlanService called!\n");
	geometry_msgs::PoseStamped p = req.goal;
	p = checker_->transformPoseTo(&p, string("base_link"), false);
...
std::vector<geometry_msgs::PoseStamped> upo_RRT_ros::RRT_ros_wrapper3::simple_path_smoothing(std::vector<geometry_msgs::PoseStamped>* path)
{
...
std::vector<geometry_msgs::PoseStamped> upo_RRT_ros::RRT_ros_wrapper3::path_interpolation(std::vector<geometry_msgs::PoseStamped> path, float step_distance)
{
...
void upo_RRT_ros::RRT_ros_wrapper3::setBiasingPath(std::vector<geometry_msgs::PoseStamped>* path_to_follow) {
...
void upo_RRT_ros::RRT_ros_wrapper3::setPathPredictionBiasing() {
	
	vector<upo_RRT::State> points = checker_->getPredictedPointList();
	//printf("\nRRT_rosWrapper3. SetPathPredictionBiasing. path size: %u\n\n", (unsigned int)points.size());
	rrt_planner_->setBiasingPath(&points);
}
...
float upo_RRT_ros::RRT_ros_wrapper3::get_rrt_planning_radius() { return size_x_; }



void upo_RRT_ros::RRT_ros_wrapper3::visualizeTree(ros::Time t) 
{
...
/*
void upo_RRT_ros::RRT_ros_wrapper3::publish_feature_costmap(ros::Time t)
  {
...
float upo_RRT_ros::RRT_ros_wrapper3::get_path_cost()
{

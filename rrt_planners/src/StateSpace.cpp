...
RRT::StateSpace::StateSpace(StateChecker* stateChecker, unsigned int dim, unsigned int dim_type, float sx, float sy, float sz, float xyzres, 
	float yawres, float min_lv, float max_lv, float lvres, 
	float max_av, float avres)
	: dimensions_(dim), type_(dim_type), size_x_(sx), size_y_(sy), size_z_(sz), xyz_resolution_(xyzres), 
	rpy_resolution_(yawres), min_lin_vel_(min_lv), max_lin_vel_(max_lv),
...
/*
RRT::State* RRT::StateSpace::samplePathBiasing(std::vector<State>* path, float stddev, float yawdev) {
...
RRT::State* RRT::StateSpace::sampleStateExternal()
{
	...
	if(!external_samples_.empty())
	{
		...
		do {
			...
			//printf("StateSpace. sampled index %i. ok: %i, total: %u\n", ind, (int)ok, (unsigned int)external_samples_.size());
...
unsigned int RRT::StateSpace::getDimensions() {return dimensions_;}
std::vector<unsigned int> RRT::StateSpace::getWeights() {return weights_;}
float RRT::StateSpace::getSizeX() {return size_x_;}
float RRT::StateSpace::getSizeY() {return size_y_;}
float RRT::StateSpace::getXYZresolution() {return xyz_resolution_;}
float RRT::StateSpace::getRPYResolution() {return rpy_resolution_;}
float RRT::StateSpace::getMinLinVel() {return min_lin_vel_;}
float RRT::StateSpace::getMaxLinVel() {return max_lin_vel_;}
float RRT::StateSpace::getLinVelResolution() {return lin_vel_res_;}
float RRT::StateSpace::getMaxAngVel() {return max_ang_vel_;}
float RRT::StateSpace::getAngVelResolution() {return ang_vel_res_;}
float RRT::StateSpace::getGoalXYZTolerance() {return goal_xyz_tolerance_;}
float RRT::StateSpace::getGoalTHTolerance() {return goal_th_tolerance_;}
RRT::State* RRT::StateSpace::getStart() {return start_;}
RRT::State* RRT::StateSpace::getGoal() {return goal_;}


void RRT::StateSpace::setDimensions(unsigned int d) {dimensions_=d;}
void RRT::StateSpace::setWeights(std::vector<unsigned int> w) {
	weights_.clear();
	for(unsigned int i=0; i<w.size(); i++)
		weights_.push_back(w[i]);
}
void RRT::StateSpace::setSizeX(float sx) {size_x_=sx;}
void RRT::StateSpace::setSizeY(float sy) {size_y_=sy;}
void RRT::StateSpace::setXYZresolution(float res) {xyz_resolution_=res;}
void RRT::StateSpace::setRPYResolution(float res) {rpy_resolution_=res;}
void RRT::StateSpace::setMinLinVel(float v) {min_lin_vel_=v;}
void RRT::StateSpace::setMaxLinVel(float v) {max_lin_vel_=v;}
void RRT::StateSpace::setLinVelResolution(float res) {lin_vel_res_=res;}
void RRT::StateSpace::setMaxAngVel(float v) {max_ang_vel_=v;}
void RRT::StateSpace::setAngVelResolution(float res) {ang_vel_res_=res;}

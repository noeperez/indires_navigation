...
void RRT::Planner::setup(StateChecker* sch, unsigned int nn_params, unsigned int dim, unsigned int dim_type, float sx, float sy, float sz, float xyzres, float yawres, 
				...
				float steer_kp, float steer_kv, float steer_ka, float steer_ko) 
{
	...
	space_ = new StateSpace(sch, dim, dim_type, sx, sy, sz, xyzres, 
...
bool RRT::Planner::setStartAndGoal(float start_x, float start_y, float start_z, float start_h, float goal_x, float goal_y, float goal_z, float goal_h)
{
...
void RRT::Planner::storeTree(std::vector<Node*> list) {
	...
	//printf("Store tree. list %u, tree: %u, fathers: %u, leaves:%u\n", (unsigned int)list.size(), (unsigned int)tree_.size(), fathers, leaves);
}


std::vector<RRT::Node*> RRT::Planner::storeLeafNodes(std::vector<Node*> list) {
	
	std::vector<Node*> leaves_pointers;
	//nn_->list(nodes);

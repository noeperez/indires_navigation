...
bool RRT::RRTstar::steer(Node* fromNode, Node* toNode, Node* newNode)
{
...
bool RRT::RRTstar::collisionFree(Node* fromNode, Node* toNode, std::vector<Action>& acts, std::vector<State>& istates, float& motCost) {
...
std::vector<RRT::Node> RRT::RRTstar::solve(float secs)
{
	...
	***********************************************************************************************/
	...
	while(!end)
	{
		...
		if(steering_->steer2(nearNode, &randNode, newNode))
		{
			...
			{
				if(nbrs[i]!=nearNode && collisionFree(nbrs[i], newNode, acts_aux, i_aux, aux_inc_cost)) 
				{
			...
			for(unsigned int i=0; i<nbrs.size(); i++)
			{
				if(nbrs[i]!=node_min && collisionFree(newNode, nbrs[i], acts2, istates, aux_inc_cost))  
				{
					posible_rewirings++;
...
void RRT::RRTstar::getNearestNeighbors(Node* node, std::vector<Node*> &nbrs) 
{
	...
	if (useKnearest_)
	{
		...
		if(nbrs.size() == 0)
			printf("Tree size: %.0f. No neighbors found!!!!\n", size);
		
	} else {
		// Nearest in radius RRT*
		double rad = r_rrt_ * std::pow(log(size) / size, 1 / static_cast<double>(space_->getDimensions()));
		double r = std::min((double)maxRange_, rad);
		nn_->nearestR(node, r, nbrs);
		//printf("Radius: %.2f\n", r);
	}
}

void RRT::RRTstar::calculateParamsNearest() {
	...
	if(useKnearest_)  {
		...
		//printf("CalculateParamNearest. Dim: %.0f, free_volume: %.3f, unitBall: %.3f, r_rrt_: %.3f\n", dim, free_volume, unitBall, r_rrt_);
	}
}

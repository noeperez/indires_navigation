...
bool RRT::RRTstar::collisionFree(Node* fromNode, Node* toNode, std::vector<Action>& acts,
...
std::vector<RRT::Node> RRT::RRTstar::solve(float secs)
{
  ...
  ***********************************************************************************************/
  ...
  while (!end)
  {
    ...
    // if(newNode != NULL)
    if (steering_->steer2(nearNode.get(), &randNode, &newNode))
    {
...
void RRT::RRTstar::getNearestNeighbors(KDTree<RRT::Node>& nn, const Node& node,

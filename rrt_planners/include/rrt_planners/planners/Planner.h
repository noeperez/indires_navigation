...
namespace RRT
{
	class Planner
	{
		...
		public:
			...
			virtual RRT::State* steer(State* fromState, State* toState) { return NULL; }
			
			virtual RRT::Node* steer(Node* fromState, Node* toState) { return NULL; }
			
			
			void setup(StateChecker* sch, unsigned int nn_params, unsigned int dim, unsigned int dim_type, float sx, float sy, float sz=0.0, float xyzres = 0.1, float yawres = 0.02, 
			...
			bool setStartAndGoal(float start_x, float start_y, float start_z, float start_h, float goal_x, float goal_y, float goal_z, float goal_h);
			...
			std::vector<Node*> storeLeafNodes(std::vector<Node*> list);

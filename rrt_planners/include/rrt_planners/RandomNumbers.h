...
public:
  ...
  /** \brief Generate a random integer within given bounds: [\e lower_bound, \e
   * upper_bound] */
  ...
  /** \brief Generate a random real using a normal distribution with given mean and
   * variance */
  ...
  /** \brief Generate a random real using a half-normal distribution. The value is within
     specified bounds [\e
      r_min, \e r_max], but with a bias towards \e r_max. The function is implemended
     using a Gaussian distribution with
      mean at \e r_max - \e r_min. The distribution is 'folded' around \e r_max axis
     towards \e r_min.
      The variance of the distribution is (\e r_max - \e r_min) / \e focus. The higher the
     focus,
  ...
  /** \brief Uniform random unit quaternion sampling. The computed value has the order
   * (x,y,z,w) */
  void quaternion(double value[4]);

  /** \brief Uniform random sampling of Euler roll-pitch-yaw angles, each in the range
   * (-pi, pi]. The computed value has the order (roll, pitch, yaw) */
